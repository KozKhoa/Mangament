...
export default function RatingCommentBox({ type, isEdited, value, onChange, className }: RatingCommentProps) {
    ...
    <div className={`w-full h-full flex flex-col gap-1 ${className}`}>
      ...
      <div className="flex flex-col gap-1">
        ...
        {isEdited ? (
          ...
          <p className="w-full h-full min-h-10 outline-none p-2 border-2 border-foreground rounded-md">{value?.message}</p>
        )}
      </div>
    </div>
  );
